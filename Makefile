...
ISR_VECTOR_COUNT := 1
MEMORY_SIZE := 32K
LDSCRIPT := pic32mx2xx_uhi.ld

include ${MIPS_ELF_ROOT}/share/mips/rules/mipshal.mk

SRCS := main.c pic32_cpy_data.c pic32mx_config.c p32mx250f128b.S excpt_isr.S
SRCS += reset_mod.S
RM := rm -rf
MKDIR := mkdir -p
BINDIR ?= bin
...
CFLAGS += -march=m4k -EL -msoft-float -g3 -O2 -Werror -Wall -Wextra -Wpedantic
CFLAGS += -Wconversion -Wcast-align -Wint-to-pointer-cast -Wstrict-prototypes
CFLAGS += -Wcast-qual -Wundef -Wstrict-overflow=5 -Wwrite-strings -Wshadow
CFLAGS += -Wfloat-equal -pedantic-errors -std=c11 $(IDIRS:%=-I%/)
CFLAGS += $(DEFINES:%=-D%)
LDFLAGS += -march=m4k -EL -msoft-float -Wl,--defsym,__use_excpt_boot=0 -g3 -O2
